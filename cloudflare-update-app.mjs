...
async function main() {
  ...
  for (const r of records.result) {
    if (r.type === "CNAME" && r.name === "app.menubuildr.com") {
      console.log(`Current: ${r.name} -> ${r.content}`);
      console.log(`Updating to: 1lqxxtde.up.railway.app`);
      const result = await cfAPI(`/zones/${ZONE_ID}/dns_records/${r.id}`, "PATCH", {
        content: "1lqxxtde.up.railway.app",
      });
