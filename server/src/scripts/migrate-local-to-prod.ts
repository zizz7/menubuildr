 ...
 * Re-maps local admin_id -> prod admin@menubuildr.com id
 */
...
async function main() {
  ...
  console.log('Prod admin ID:', prodAdmin.id);
  ...
  console.log(`Local: ${restaurants.length} restaurants, ${menus.length} menus, ${sections.length} sections, ${items.length} items`);
  ...
  for (const m of menus) {
    ...
    if (!exists) { await prod.menu.create({ data: m as any }); }
  }
  ...
  console.log('  Menu versions: done');
  ...
  for (const item of items) {
    const { allergens: ia, recipeDetails: rd, priceVariations: pv, availabilitySchedule: as_, translations: tr, ...data } = item;
    ...
    if (as_) { await prod.availabilitySchedule.create({ data: as_ as any }); }
