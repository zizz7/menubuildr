...
$backendCmd = "`$env:PORT='$backendPort'; cd '$backendPath'; Write-Host 'Backend Server - Port $backendPort' -ForegroundColor Cyan; npm run dev"
...
$frontendCmd = "cd '$frontendPath'; Write-Host 'Frontend Server - Port $frontendPort' -ForegroundColor Cyan; `$env:PORT='$frontendPort'; npm run dev -- -p $frontendPort"
...
Write-Host "   - Run database setup if needed: cd server; npm run db:push; npm run db:seed" -ForegroundColor Gray
Write-Host ""
